...
For our first insight, we wanted to see if the occurrence of suicide was similar or different for men and women. To do this, we grouped the data by sex and piped it into a ggplot bar graph.
...
From our data, we found that men commit suicide about **3.5 times more than women**. 
...
For our third insight, we wanted to determine if a country's economic status had any influence on their suicide rate. Intuition would suggest that a richer country would have a lower rate of suicide. To challenge this intuition, we ____________________.
...
suicide %>% 
  ...
  ggplot(aes(x = reorder(country, -sui_100k), y = sui_100k, fill = country)) + 
  geom_bar(stat="identity") +
  labs(title = "Relationship between GDP and suicide (per 100k population)", x = "GDP per capita", y = "Suicides per 100k Population") +
  theme(axis.text.x = element_text(angle=50, hjust=1))

```

From our graph, we see that there is a ______ relationship between a country's GDP and their suicide per 100k population. 
